...
- **金叉买入**：DIF 上穿 DEA，次日开盘价执行
- **死叉卖出**：DIF 下穿 DEA，次日开盘价执行
- **交易成本**：单边 0.1%（千一），往返 0.2%
...
> 策略净值从 1.000 增长至 **4.574**，同期基准净值仅为 **2.145**。
...
`run_signal.py` 会自动从新浪财经下载最新的日线数据，计算 MACD 信号并生成 `docs/signal_data.json`。页面内嵌了备份数据，**本地双击 `index.html` 也能正常显示**。
...
| 触发条件 | 时间 | 用途 |
|:---|:---|:---|
| 收盘更新 | 每个交易日 15:30 | 下载最新日线数据，生成当日信号 |
